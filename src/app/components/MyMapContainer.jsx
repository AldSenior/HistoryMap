...
function MyMapComponent({ events }) {
	...
	const handleWheel = e => {
		...
		const newTranslateX = Math.max(
			0,
			Math.min(
				rect.width - rect.width * newScale,
		...
		const newTranslateY = Math.max(
			0,
			Math.min(
				rect.height - rect.height * newScale,
					...
					<AnimatePresence>
						{filteredEvents.map(event => (
							<motion.div
								...
								className={`absolute cursor-pointer rounded-full w-7 h-7 bg-blue-500 ${
									hoveredMarkerId === event.id ? 'scale-150' : ''
								}`}
								id='marker'
								style={{
